...
# 2) Install Python + smartmontools (portable fallback) and refresh PATH for THIS layer
RUN choco install -y python --no-progress ; `
    try { choco install -y smartmontools --no-progress } catch { choco install -y smartmontools.portable --no-progress } ; `
    [Environment]::SetEnvironmentVariable('Path','C:\Python311;C:\Python311\Scripts;C:\Program Files\smartmontools\bin;C:\tools\smartmontools\bin;C:\ProgramData\chocolatey\bin;' + [Environment]::GetEnvironmentVariable('Path','Process'),'Process') ; `
...
# 4) App files (your existing POSIX-style paths are fine)
WORKDIR /app
COPY scripts /app/scripts
COPY db /app/db
COPY logs /app/logs
